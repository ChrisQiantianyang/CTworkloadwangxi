...
library(caret)
library(fpp3)
datact = read_csv("D:/task/CTworkload/shiyan/datacount8zengqiang.csv")
plot = datact %>%
  plot_time_series(ds,y)
datatrain = datact %>%
  filter(ds<as_date("2023-08-01"))
datatest = datact %>%
  filter(ds>=as_date("2023-08-01")&ds<as_date("2023-09-01"))
newyearsday = tibble(
  holiday = "newyearsday",
...
cty = calibration_tbl$.calibration_data[[1]]
y_true <- exp(cty$.actual)
y_pred <- exp(cty$.prediction)
r_squared <- postResample(y_pred, y_true)
r_squared
r_squared2 <- postResample(cty$.prediction, cty$.actual)
r_squared2
cty1 = cty %>%
  mutate(.actual = exp(cty$.actual)) %>%
  mutate(.prediction = exp(cty$.prediction)) %>%
  mutate(.residuals = .prediction-.actual) %>%
